...
- **A merge that spreads then re-lists is an allow-list — treat every such literal as a standing hazard.** `merge_into_existing` (`builders/merge.ts`) builds `{...existing, state, init, events, actions, patch, on, given, snap}`, so any `State` field not named there silently keeps the FIRST partial's value and discards the incoming one (#1645: `options`, `disclose`, `autoclose`, `archive`, and the two day fields). This is the same shape as wave 22's run-once-block rule: **when a new field lands on `State`, check the merge literal**, and when auditing, ask what the spread is quietly deciding rather than what the explicit list says. `pii_aware`/`message`/`view` escape it only because `build_events` re-derives them after the merge.
- **`wire_safe` is a WIRE transform, not a state transform (act-http/sse).** It converts `undefined`→`null` and rebuilds plain objects via `Object.entries`, which **drops symbol keys**. So it cannot be applied naively to cached state: doing so breaks #1471 (behavior-contract row 190 promises server-side cached state is untouched — a reseed has the key absent, not null) and #1473 (the `OVERLAY_KEYS` symbol carry). `overlay()` gets away with normalizing before its cache write only because it normalizes the *patch* (where `apply_patch` treats `null` and `undefined` alike) and then re-tags. Any fix for #1646 has to preserve both.
- **`??` over a config field whose schema admits `0` is a bug pattern.** `leaseMillis: 0` is legal (`z.number().min(0).optional()`, established reachable by #1600), so `x ?? DEFAULT` silently takes the `0` where `x || DEFAULT` was meant (#1647, the #1617 fold's other operand). When a numeric option's floor is `0` rather than `1`, `??` is almost always wrong at a *fallback* site — check whether `0` is a real value or an absence.
...
- **Sweep the WHOLE tree at the end of a wave, not each lens's package.** Wave 23 found a probe left in `packages/calculator/test/` by a hunter whose lens was elsewhere, and two hunters independently chose the filename `_probe_sib.spec.ts` in different packages. Start every wave from a clean `git status` so strays are unambiguous, and copy probes out under distinct names before deleting.
- **Re-read every hunter notes file at wave close, even ones you already incorporated.** In wave 23 the toolchain hunter appended its single best finding (#1652) to a notes file the lead had already read and summarized, then went idle — the interim report told the user that lens was clean, and it wasn't. An idle notification means "check the file again", not "the file is final". Compare sizes/mtimes against what you read.
