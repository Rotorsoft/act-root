...
// node-pg returns int8 as a STRING by default, to avoid silently losing
// precision past 2^53. `events.id` is `bigserial` since #1422, and
// `Committed.id` is a JS `number` — so without this parser every id would
// arrive as a string and every id comparison in the orchestrator would fail.
// Numbers are exact to 2^53, which is four orders of magnitude past the
// int4 ceiling the widening was for, so the coercion is safe in practice.
const INT8_OID = types.builtins.INT8;
const scopedTypes: { getTypeParser: typeof types.getTypeParser } = {
  getTypeParser: ((oid: number, format?: unknown) => {
    if (oid === JSONB_OID) return (val: string) => JSON.parse(val, dateReviver);
    if (oid === INT8_OID) return (val: string) => Number(val);
    return (types.getTypeParser as (oid: number, format?: unknown) => unknown)(
      oid,
      format
    );
...
export class PostgresStore implements Store {
  ...
  async seed() {
    ...
    try {
      ...
      // Migration for tables created before the id widening (#1422). `serial`
      // is int4, capping an append-only store at ~2.1e9 events — SQLite uses
      // a 64-bit INTEGER and InMemory a JS number. At the ceiling every
      // commit fails permanently and there is no in-place recovery, since
      // ALTER on a 2.1-billion-row table is a full rewrite. `Committed.id` is
      // a JS number (safe to 2^53), so the wider column strains nothing.
      // Guarded on the current type: the rewrite happens once, and is far
      // cheaper at 10M rows than at the cliff.
      await client.query(
        `DO $$
         BEGIN
           IF EXISTS (
             SELECT 1 FROM information_schema.columns
             WHERE table_schema = '${this.config.schema}'
               AND table_name = '${this.config.table}'
               AND column_name = 'id'
               AND data_type = 'integer'
           ) THEN
             EXECUTE 'ALTER TABLE ${this._fqt} ALTER COLUMN id TYPE bigint';
             EXECUTE 'ALTER SEQUENCE "${this.config.schema}"."${this.config.table}_id_seq" AS bigint';
           END IF;
         END
         $$;`
      );
