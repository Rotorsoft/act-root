...
export type EventTags = {
  ...
  /**
   * Revives the dates in a stored payload, or `undefined` when the schema
   * declares none.
   */
  readonly parse: ((data: unknown) => unknown) | undefined;
  /** The same for the `pii` sidecar, when a sensitive field is a date. */
...
/**
 * Build a schema that describes an event's date fields and nothing else, or
 * `undefined` when it declares no dates.
 *
 * JSON has no date type, so a stored `Date` comes back as text and something
 * has to turn it back. That is the entire job. The payload was validated when
 * it was committed, so every other field is already known to be good and is
 * left out — it rides through the loose object untouched rather than being
 * checked a second time.
 *
 * Leaving them out is also what stops a read from rejecting what the framework
 * itself wrote. A `sensitive(...)` field is moved into the `pii` sidecar on the
 * way in, so `data` does not hold it; an event written against an older
 * declaration predates whatever was added since. Neither is a date, so neither
 * is this schema's business. The dates themselves are optional for the same
 * reason: absent is not wrong.
 *
 * Zod still does the walking, so nesting, arrays, records, unions and the
 * wrappers are handled by the engine rather than by a traversal of our own
 * that would drift as Zod grows constructs. A construct this doesn't
 * recognise contributes no date, which is the documented fallthrough.
 */
function to_date_schema(
  schema: unknown,
  sensitive?: readonly string[]
): z.ZodType | undefined {
  const def = def_of(schema);
  if (!def) return undefined;
  const inner = () => to_date_schema(def.innerType);
  switch (def.type) {
    case "date":
...
export function event_tags(schema: z.ZodType): EventTags {
  ...
  const collect = (node: unknown): void => {
    ...
    if (shape) {
      for (const key of Object.keys(shape))
        if (is_pii(shape[key])) {
          sensitive.push(key);
          const dates = to_date_schema(shape[key]);
  ...
  const data_schema = to_date_schema(schema, unique);
  ...
  return {
    sensitive: unique,
    parse: data_schema && revive(data_schema),
    parse_pii: has_pii_date ? revive(z.looseObject(pii_dates)) : undefined,
  };
}
