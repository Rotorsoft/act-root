 ...
 *
 * `build_handle` installs it around each lease; `action()` resolves it as
 * `reactingTo` when the caller didn't pass one, so a handler that dispatches
 * through a captured `app` still threads the chain.
 *
 * @internal
 */
