      ...
      (event) => {
        ...
        if (register) {
          for (const reaction of register.reactions.values()) {
            ...
            // Let a target through when it's new OR a later scan resolves a
            // strictly higher priority than it was last subscribed at, so the
            // store's `GREATEST` upsert raises it — the documented runtime
            // `max()` invariant, which the plain "already subscribed?" dedup
            // silently froze at first discovery (#1363).
            const recorded = resolved
              ? this._subscribed.get(resolved.target)
              : undefined;
            if (
              resolved &&
              (recorded === undefined || (resolved.priority ?? 0) > recorded)
            ) {
