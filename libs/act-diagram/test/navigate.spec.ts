...
import { navigate_to_code } from "../src/client/lib/navigate.js";
...
describe("navigate_to_code", () => {
  it("finds a state definition", () => {
    const result = navigate_to_code(SAMPLE_FILES, "Ticket", "state");
  ...
  it("finds an action definition", () => {
    const result = navigate_to_code(SAMPLE_FILES, "OpenTicket", "action");
    expect(result).toBeDefined();
    expect(result!.file).toBe("src/states.ts");
  });

  it("finds an event definition", () => {
    const result = navigate_to_code(SAMPLE_FILES, "TicketOpened", "event");
    expect(result).toBeDefined();
    expect(result!.file).toBe("src/states.ts");
  });

  it("finds a reaction handler", () => {
    const result = navigate_to_code(SAMPLE_FILES, "autoAssign", "reaction");
    expect(result).toBeDefined();
    expect(result!.file).toBe("src/slices.ts");
  });

  it("finds a projection by variable name", () => {
    const result = navigate_to_code(
      SAMPLE_FILES,
      "TicketProjection",
      "projection"
    );
  ...
  it("finds a projection by string name", () => {
    const result = navigate_to_code(SAMPLE_FILES, "tickets", "projection");
    expect(result).toBeDefined();
    expect(result!.file).toBe("src/proj.ts");
  });
  ...
  it("finds guard by variable name", () => {
    const result = navigate_to_code(SAMPLE_FILES, "mustBeOpen", "guard");
    expect(result).toBeDefined();
    expect(result!.file).toBe("src/states.ts");
  });

  it("navigates to a file with act() call", () => {
    const result = navigate_to_code(SAMPLE_FILES, "src/app.ts", "file");
    expect(result).toBeDefined();
  ...
  it("navigates to a file without act() — returns line 1", () => {
    const result = navigate_to_code(SAMPLE_FILES, "src/states.ts", "file");
    expect(result).toBeDefined();
    expect(result!.file).toBe("src/states.ts");
  ...
  it("file navigation with partial path match", () => {
    const result = navigate_to_code(SAMPLE_FILES, "app.ts", "file");
    expect(result).toBeDefined();
    expect(result!.file).toBe("src/app.ts");
  });
  ...
  it("file navigation returns undefined for non-existent file", () => {
    expect(
      navigate_to_code(SAMPLE_FILES, "nonexistent.ts", "file")
    ).toBeUndefined();
  });

  it("finds name within target_file using event block priority", () => {
    const result = navigate_to_code(
      SAMPLE_FILES,
      "TicketOpened",
      "event",
  ...
  it("finds name within target_file using action block priority", () => {
    const result = navigate_to_code(
      SAMPLE_FILES,
      "OpenTicket",
      "action",
  ...
  it("target_file with no block match falls back to generic search", () => {
    const result = navigate_to_code(
      SAMPLE_FILES,
      "Ticket",
      "state",
  ...
  it("target_file with name not found returns undefined", () => {
    expect(
      navigate_to_code(SAMPLE_FILES, "NonExistent", "event", "src/states.ts")
    ).toBeUndefined();
  });

  it("target_file not in file list returns undefined", () => {
    expect(
      navigate_to_code(SAMPLE_FILES, "Ticket", "state", "src/nonexistent.ts")
    ).toBeUndefined();
  });

  it("returns undefined for non-existent element", () => {
    expect(
      navigate_to_code(SAMPLE_FILES, "DoesNotExist", "state")
    ).toBeUndefined();
  });

  it("generic search (no type) finds across all patterns", () => {
    const result = navigate_to_code(SAMPLE_FILES, "TicketSlice");
    expect(result).toBeDefined();
    expect(result!.file).toBe("src/slices.ts");
  });
...
export const Ticket = state({ Ticket: z.object({}) }).init(() => ({})).emits({}).build();`,
    ...
    const result = navigate_to_code(files, "Ticket", "state");
...
export const Real = state({ Real: z.object({}) }).init(() => ({})).emits({ D: z.object({}) }).on({ X: z.object({}) }).emit("D").build();`,
    ...
    expect(navigate_to_code(files, "Commented", "state")).toBeUndefined();
  });
...
export const R = state({ R: z.object({}) }).init(() => ({})).emits({ D: z.object({}) }).on({ X: z.object({}) }).emit("D").build();`,
    ...
    expect(navigate_to_code(files, "InBlock", "state")).toBeUndefined();
  });
...
export const R = state({ R: z.object({}) }).init(() => ({})).emits({}).build();`,
    ...
    expect(navigate_to_code(files, "InJSDoc", "state")).toBeUndefined();
  });
...
export const R = state({ R: z.object({}) }).init(() => ({})).emits({}).build();`,
    ...
    expect(navigate_to_code(files, "Hidden", "state")).toBeUndefined();
  });
...
export const Found = state({ Found: z.object({}) }).init(() => ({})).emits({}).build();`,
    ...
    const result = navigate_to_code(files, "Found", "state");
    expect(result).toBeDefined();
...
export const Foo = state({ Foo: z.object({}) }).init(() => ({})).emits({}).build();`,
    ...
    const result = navigate_to_code(files, "Foo", "state");
    expect(result).toBeDefined();
  ...
  it("handles match at the very start of file (col 1)", () => {
    ...
    const result = navigate_to_code(files, "Foo");
    expect(result).toBeDefined();
    expect(result!.line).toBe(1);
  });

  it("name_offset_in_match negative falls back to match.index", () => {
...
const x = state({ X: z.object({}) }).init(() => ({})).emits({}).build();`,
    ...
    const result = navigate_to_code(files, "X", "state");
    expect(result).toBeDefined();
  });

  it("guard navigation with target_file finds const declaration (lines 195-215)", () => {
    const files: FileTab[] = [
      {
        path: "src/guards.ts",
...
export const mustBeAssigned: Invariant<{ assignedTo: string }> = {
  ...
  it("guard navigation with target_file falls back to description when no const found (lines 213-215)", () => {
    const files: FileTab[] = [
      {
        path: "src/guards.ts",
        ...
        content: `// An inline guard object without a preceding const declaration
{
  ...
  valid: (s: any) => s.active,
    ...
    // No target_file — uses general search path
    const result = navigate_to_code(files, "Inline guard check", "guard");
    expect(result).toBeDefined();
...
const TicketOpened = "event";`,
    ...
    const result = navigate_to_code(files, "TicketOpened", "event", "src/t.ts");
    expect(result).toBeDefined();
    // Should find .emits on line 2, not the commented one on line 1
    expect(result!.line).toBe(2);
  });

  it("event in target_file: name exists but no .emits() call (line 183 false)", () => {
    const files: FileTab[] = [
      {
        path: "src/t.ts",
...
// no .emits() call anywhere`,
  ...
  it("guard in target_file: description not found (line 194 false)", () => {
    const files: FileTab[] = [
      {
        path: "src/guards.ts",
  ...
  it("action in target_file: no .on() block (line 220 false)", () => {
    const files: FileTab[] = [
      {
        path: "src/t.ts",
    ...
    const result = navigate_to_code(files, "MyAction", "action", "src/t.ts");
    expect(result).toBeDefined();
    // Falls through to generic fallback
    expect(result!.line).toBe(1);
  });

  it("action in target_file: action found after .on( (line 222 true)", () => {
    const files: FileTab[] = [
      {
        path: "src/t.ts",
...
const MyAction = "declared later";`,
  ...
  it("action in target_file: action not after .on( (line 222 false)", () => {
    const files: FileTab[] = [
      {
        path: "src/t.ts",
...
const x = builder.on({ Other: {} }).emit("E");`,
  ...
  it("generic search sorts test files after source files (line 246)", () => {
    ...
    const result = navigate_to_code(files, "MyThing");
    expect(result).toBeDefined();
  ...
  valid: (s: any) => s.ok,
    ...
    const result = navigate_to_code(files, "Some guard desc", "guard");
    expect(result).toBeDefined();
  ...
  valid: (s: any) => s.active,
    ...
    const result = navigate_to_code(files, "Must be active", "guard");
    expect(result).toBeDefined();
    expect(result!.file).toBe("src/guards.ts");
