...
So `@rotorsoft/act-ops` — a deliberately broad name, scoped for future cross-cutting operational primitives that share the same "non-Act apps want this too" property. The next siblings line up around it: the TTL math from ACT-603 (which would land alongside `IdempotencyStore` rather than as a freestanding helper — see [ACT-1119](./act-1119-retry-profile.md) for that design journey) and the retry-budget classifiers that ACT-601 hinted at. All of them are math, not orchestration. None of them need `@rotorsoft/act` at runtime. The package is the home.
