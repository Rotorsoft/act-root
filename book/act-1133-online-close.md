...
This essay covers the foundation: the declarators, the cycle, the controller. The policy-factory layer lands in #838 — but instead of a wrapping factory, `.autocloses` is overloaded to accept either a predicate function or a declarative options object with verb-shaped fields (`after` / `is` / `reaches`). The three originally-separate factory tickets (#838 retention, #839 terminal, #840 cardinality) collapsed into one when the first call site showed every real policy stacks; the wrapping factory followed when the call site read better without it. The declarative form compiles to the same `(stream, head, count) => boolean` shape the foundation primitive consumes. Operators with stacked policies write the object; operators with custom policies write the function. The declarative form is sugar; the primitive is the predicate.
