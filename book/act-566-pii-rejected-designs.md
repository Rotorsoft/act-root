...
The framework boundary flexes in one narrower place than that, and it's worth naming honestly. The adapters — `@rotorsoft/act-pg`, `@rotorsoft/act-sqlite` — already own the SQL that writes the `pii` column. A per-adapter constructor option that wraps the column on commit and unwraps it on read, with an operator-controlled key provider callback, is meaningfully lighter than the rejected framework-wide port and reaches deployments TDE can't: self-hosted Postgres without `pgcrypto`, edge SQLite on devices the operator doesn't fully control, container or serverless workloads where the volume is opaque. The `Store` contract doesn't change, the `pii_isolation` capability still gates the column itself, and `forget_pii` semantics survive intact — a `NULL` is a `NULL` whether the ciphertext or the plaintext lived there before.

That shape shipped as #921, the follow-up to the original epic. The cipher lives in `@rotorsoft/act-crypto` — a leaf-level helper that ships authenticated envelope encryption (AES-256-GCM with a versioned wire format) and nothing else. Both production adapters depend on it; the framework core does not. The boundary at "the framework does not ship encryption" remained true for `@rotorsoft/act` specifically — the core stayed unaware of cipher, key, and envelope — while the broader `@rotorsoft` family gained a per-adapter option for operators whose deployment posture sits outside the TDE / `pgcrypto` lane. The framework declining to ship encryption at the core was one decision; an adapter declining was a separate one, and the answer turned out to be "no, the adapters can ship it cleanly, because they already own the column they'd be wrapping."
