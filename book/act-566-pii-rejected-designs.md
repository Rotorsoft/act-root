...
The framework boundary flexes in one narrower place than that, though, and it's worth naming honestly. The adapters — `@rotorsoft/act-pg`, `@rotorsoft/act-sqlite` — already own the SQL that writes the `pii` column. Adding a per-adapter constructor option that wraps the column on commit and unwraps it on read, with an operator-controlled key provider callback, is meaningfully lighter than the rejected framework-wide port and reaches deployments TDE can't: self-hosted Postgres without `pgcrypto`, edge SQLite on devices the operator doesn't fully control, container or serverless workloads where the volume is opaque. The `Store` contract doesn't change, the `pii_isolation` capability still gates the column itself, and `forget_pii` semantics survive intact — a `NULL` is a `NULL` whether the ciphertext or the plaintext lived there before. The framework declining to ship encryption at the core is one decision; an adapter declining to ship it is a separate one the framework didn't yet need to make. That ticket is open as #921, scoped narrowly to the per-adapter shape; until it lands, the operator-side recipes in the encryption guide are the answer, and the boundary at "framework does not ship encryption" is true for `@rotorsoft/act` specifically rather than for the whole `@rotorsoft` family.
