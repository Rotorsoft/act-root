...
| 5 | **Delete the legacy probe arm** + add an opt-in reconciliation sweep. | payoff | `claim` stops touching the event log on both adapters. The conservative "unmarked row = pending" arm in the close-cycle probe goes with it. |
