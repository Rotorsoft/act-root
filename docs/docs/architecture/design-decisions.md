...
Act has no *first-class* scheduling or timeout primitive yet. The classic pattern — "if no payment within thirty minutes, emit `OrderExpired`," a saga waiting on the *absence* of an event — has no public surface today. Peers all ship one (Axon's `DeadlineManager`, NServiceBus and MassTransit saga timeouts, Temporal timers, EventStoreDB scheduled messages). This is not a deliberate non-goal the way the three above are; it is missing, and the design stance for closing it is settled: model the schedule as event-sourcing-native data, fire it from the drain's due-time machinery, add no cron dependency.

The foundation for that machinery now exists. [#1090](https://github.com/Rotorsoft/act-root/issues/1090) gave the drain a **deferred-reaction outcome**: a handler can park its stream until a future wall-clock time without acking (consuming the event) or failing (burning a retry). The due-time is persisted via `Store.defer`, and `claim` skips a deferred stream until it comes due, so the deferral is durable shared state across every worker rather than in-process pacing. That outcome is what `.autocloses` rides — it is no longer a single-purpose time-windowed sweep but a synthesized reaction that defers across the cooldown and then closes through the ordinary `run_close_cycle` path. The remaining work is the public, event-sourced *schedule* surface on top of that primitive (a saga setting and cancelling its own deadlines), tracked at [#1049](https://github.com/Rotorsoft/act-root/issues/1049). When you reach for a user-facing timer today and find only the internal defer outcome, that is the gap — not another choice.
