...
`QueryStatsOptions` also carries the keyset-pagination pair `after`/`limit`. Results are ordered by stream name ascending; `after` is an **exclusive** cursor (the page contains streams sorting strictly after it), and the next cursor is the last key of the returned map (`[...result.keys()].at(-1)`). The one wrinkle to watch: `limit` defaults to **unbounded** — omit it and `query_stats` returns every matching stream, exactly as it did before pagination existed. This is the opposite of `query_streams.limit`, which defaults to 100. The autoclose cycle relies on this pair to sweep one bounded page per tick instead of scanning every stream (see [Close cycle § Online close-the-books](./close-cycle.md#online-close-the-books-837--epic-802)).

`query_streams` gained a best-effort narrowing filter, `source_matches`. Where the existing `source` filter matches subscriptions whose `source` pattern is matched *by* a given value (`source ~ pattern`), `source_matches` runs the relation in reverse: prefer subscriptions whose stored `source` pattern matches at least one of the supplied stream names (`name ~ source`). The close-cycle safety probe uses it to fetch only the subscriptions that could possibly be reacting to the streams it's about to close. Two subtleties make it a hint rather than an exact filter. First, a subscription with an **absent or empty** `source` carries no source constraint — it consumes from every stream — so it *always* qualifies regardless of the names passed. Second, a store that can't express reverse-regex is free to ignore the filter and return a superset; the safety probe re-verifies source and target in process, so correctness never depends on the store honoring it. It's capability-gated as `source_matches` in the TCK — the gate covers the *narrowing* tests, not the probe's correctness. `PostgresStore` and `InMemoryStore` implement it natively; `SqliteStore` declines it and falls back to the exhaustive probe.
