...
The `EventSource.query` callback is typed `(event) => void`, but adapters wrap each invocation in `await Promise.resolve(callback(event))`. TypeScript's "any return ignored when the type says `void`" rule lets the same call site work for both sync (`e => arr.push(e)`, returns `number`) and async (`async e => …`) callbacks. The orchestrator's `iterate` utility (in `libs/act/src/internal/event-sourcing.ts`, alongside `scan`) exploits this seam: it returns a promise from its callback that resolves only when the consumer pulls the event, turning the sync-callback `query` API into an `AsyncIterable<Committed>` with a 1-slot mailbox. The producer fills the slot, the consumer drains it, and `await callback(event)` blocks the next read until the consumer is ready. `iterate` is internal because it's a transport detail; `CsvFile`, `EventSource`, and `EventSink` are the public surface the rest of the framework speaks.

The mailbox keeps memory flat **downstream** of the producer, but each adapter still chooses how its own `query` materializes the source rows. Two paths:

- **Buffered (default).** The adapter resolves the full result set into a JS array before the first callback fires (`pool.query` in PG, the in-memory array in InMemory, the libsql result set in SQLite). Cheapest in latency — one round trip — and the right path for every call site that already passes a bounded `limit`: aggregate `load()`, projection scans, inspector pages.
- **Streaming (opt-in).** The caller sets `streaming: true` on the `Query` filter. Adapters that support it switch to a cursor-backed read so producer memory stays O(`batch_size`) instead of O(rowCount). `PostgresStore` opens a `pg-cursor` inside a transaction and FETCHes in batches (default 500 rows). InMemory and SQLite ignore the flag — their internal representation is already bounded — and the buffered path costs the same. Added in [ACT-1132](https://github.com/Rotorsoft/act-root/issues/814).

The orchestrator's `scan` (used by `Act.restore` / `Act.transfer`) sets `streaming: true` unconditionally. Every other internal `query` call site (`load`, `query_streams`, `query_stats`, the per-stream replay inside `action`) stays on the buffered fast path. External callers walking an unbounded result set should set the flag explicitly; bounded callers should leave it off.
