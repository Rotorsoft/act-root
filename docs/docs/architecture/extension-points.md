...
- `drop_closed_streams` — drop every event from streams that have a `__tombstone__` event ([ACT-1126](https://github.com/Rotorsoft/act-root/issues/785)). Scan walks the source once upfront with a tombstone-name filter to collect closed-stream names, then the main pass drops every event whose stream is in the set. Counted in `ScanResult.dropped.closed_streams`. There is no `drop_empty_streams` counterpart — empty streams have zero events and never appear in an event scan to begin with.
