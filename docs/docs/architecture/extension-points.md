...
`restore` is the offline wipe-and-commit primitive (added in [ACT-1124](https://github.com/Rotorsoft/act-root/issues/783)). Capability-gated — adapters that can't atomically wipe and reinsert in one transaction don't have to implement it. The TCK exercises the suite only when the adapter passes `capabilities: { restore: true }`. The adapter's job is narrow: open a transaction, wipe events + streams/subscriptions, delegate iteration to the framework's `scan` helper with a per-event commit callback, then commit or roll back. `RESTART IDENTITY` (PG) / `sqlite_sequence` reset (SQLite) / snapshot-and-swap (InMemory) reseed dense ids from 1 (or 0). `created` is preserved verbatim — distinct from `commit`, which always stamps `now()`. Reactions re-subscribe via the orchestrator on the next settle cycle.

Everything else — iterating the source, validating each event, applying `drop_snapshots`, firing `on_progress`, building the per-call `old → new` id map and rewriting `meta.causation.event.id` so causation chains survive the renumber — lives in `scan` (libs/act/src/restore.ts), shared by every adapter. Source is an `AsyncIterable<RestoreEvent>` so multi-million-event backups don't OOM.
...
**Validation is a source operation, not a store operation.** Pre-flighting a backup — checking for malformed `created` or negative `version` — is something callers do over the source before deciding to invoke `Store.restore`. Call `scan(source)` with no `commit` callback: it walks the source, validates each event inline, and throws on the first blocker. Adapters reuse the same `scan` internally with a `commit` callback for the live write. Cross-event invariants (duplicate ids, per-stream version gaps) are not the framework's job — DB `UNIQUE(stream, version)` catches dupes at commit time, and partial backups intentionally have gaps.
