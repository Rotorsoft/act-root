...
`scan` paginates the source. Each batch calls `source.query` with `limit: 500` and `after: <last id seen>`. Stores that respect `limit` (`PostgresStore`'s `pool.query` honors it natively) hold one batch's worth of rows in memory per round trip — adapter cost is O(batch) regardless of total result size. Sources that ignore the filter and stream everything in one call (`CsvFile`) signal the loop to exit by returning more events than the requested limit; they're memory-safe because they read line-by-line internally.

A million-event PG → CSV transfer holds at most 500 rows in the adapter, one event in flight through the source's callback, and whatever the consumer accumulates downstream — independent of total source size. `CsvFile`, `EventSource`, and `EventSink` are the public surface the rest of the framework speaks.
