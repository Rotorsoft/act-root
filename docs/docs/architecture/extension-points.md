...
**Validation is a source operation, not a store operation.** Pre-flighting a backup — checking for malformed `created` or negative `version` — is something callers do over the source before deciding to invoke `Act.restore`. Call `scan(source)` with no callback: it walks the source, validates each event inline, and throws on the first blocker. The orchestrator reuses the same `scan` internally with a per-event callback for the live write. Cross-event invariants (duplicate ids, per-stream version gaps) are not the framework's job — DB `UNIQUE(stream, version)` catches dupes at commit time, and partial backups intentionally have gaps.
