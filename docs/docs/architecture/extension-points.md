...
One deliberate carve-out: the `source` a reaction resolver hands to `subscribe` is **not** one of these pattern surfaces. In `claim()`'s has-work probe it is an exact stream name compared by string equality — every shipped resolver produces exact names, and exact equality keeps the probe on the adapter's stream index instead of a regex scan ([#1182](https://github.com/Rotorsoft/act-root/issues/1182)). The TCK's "claim source matching" cases pin this on every adapter.
...
- **Exact-source claims**: `claim`'s has-work probe compares a subscription's `source` to event stream names by **string equality** — never regex, `LIKE`, or substring matching. Pattern semantics belong to the `StreamFilter` surfaces above.
