...
The `EventSource.query` callback is typed `(event) => void`, but adapters wrap each invocation in `await Promise.resolve(callback(event))`. TypeScript's "any return ignored when the type says `void`" rule lets the same call site work for both sync (`e => arr.push(e)`, returns `number`) and async (`async e => …`) callbacks ([ACT-1128](https://github.com/Rotorsoft/act-root/issues/787)). The orchestrator's `scan` (in `libs/act/src/internal/event-sourcing.ts`) exploits this seam directly: the per-event work runs inside the source's callback, so the source's own `await Promise.resolve(callback)` throttles the producer to the consumer's pace — no separate generator/mailbox bridge needed.

On top of that, `scan` paginates ([ACT-1133](https://github.com/Rotorsoft/act-root/issues/817)). Each batch calls `source.query` with `limit: 500` and `after: <last id seen>`. Stores that respect `limit` (`PostgresStore`'s `pool.query` honors it natively) hold one batch's worth of rows in memory per round trip — adapter cost stays at O(batch) regardless of total result size. Sources that ignore the filter and stream everything in one call (`CsvFile`) signal the loop to exit by returning more events than the requested limit; they're already memory-safe because they read line-by-line internally. No new dependency, no `Query` schema field, no adapter-side branching.

Net effect: a million-event PG → CSV transfer holds at most 500 rows in the adapter, one event in flight through the source's callback, and whatever the consumer accumulates downstream — independent of total source size. `CsvFile`, `EventSource`, and `EventSink` are the public surface the rest of the framework speaks.
