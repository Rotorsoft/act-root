...
`RestoreOptions` carries three flags as of [ACT-1125](https://github.com/Rotorsoft/act-root/issues/784): `drop_snapshots` (skip `__snapshot__` rows so the next snap policy regenerates them), `dry_run` (pre-flight blocker scan — adapters iterate and validate without writing, returning `errors` populated with duplicate ids / version-contiguity gaps / malformed `created` / negative versions; live restore still throws atomically on the first error so `errors` is always empty), and `on_progress` (one callback per row — callers throttle/debounce as needed). Two more flags — `drop_closed_streams` and `drop_empty_streams` — are deferred until the source-shape question (one-pass vs. re-iterable factory) is settled. The migration overlay (event-name remap, per-event transform, stream rename) lives in #785.
