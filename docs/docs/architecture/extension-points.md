...
A `Date` in an event's `data` round-trips as a `Date` on every adapter. Adapters that persist JSON revive ISO-8601-shaped strings on read — Postgres and SQLite run the same reviver, so a payload committed with a `Date` reads back as a `Date` rather than an ISO string. The documented caveat: any plain string shaped like an ISO-8601 timestamp is revived to a `Date`, and a timezone-less ISO string parses in local time. On Postgres the reviver is scoped to the store's own `Pool` via a per-Pool `types` option (delegating to the global default for every OID except JSONB) rather than mutating the process-global `pg` type parser, so the coercion never leaks into any other `pg` pool the host app runs ([#1198](https://github.com/Rotorsoft/act-root/issues/1198)).

The `source` a reaction resolver hands to `subscribe` follows a **literal fast-path / pattern** contract in `claim()`'s has-work probe. A **literal** source (no regex metacharacter — the common case, and every autoclose/dynamic-resolver source) is compared by string equality: the fast, index-friendly path, and exact so a source `s1` never matches a sibling stream `s12`. A **pattern** source (one carrying `^ $ . * + ? ( ) [ ] { } | \`, e.g. a static reaction sourcing `^(A|B)$`) is compiled as a regex and matched against candidate stream names — this restores the shipped calculator demo's regex-source Board projection that an earlier exact-only decision broke ([#1220](https://github.com/Rotorsoft/act-root/issues/1220), regressing [#1182](https://github.com/Rotorsoft/act-root/issues/1182)/#1215). Adapters that cannot faithfully run an arbitrary regex (SQLite, whose libsql build has no `REGEXP` and whose portable `LIKE` grammar cannot express alternation/grouping) reject a non-portable claim source at `subscribe` time rather than silently never claiming the stream. The TCK's "claim source matching" and "claim pattern source matching" cases pin this on every adapter.
...
- **The work set (`SubscribeInput.correlated`)**: a subscription row carries an optional **work mark** — the highest event id observed to resolve to that target ([#1485](https://github.com/Rotorsoft/act-root/issues/1485)). `subscribe` applies it as `GREATEST(correlated, N)`, so it never regresses, for every value including zero and negatives; omitting it leaves the stored value untouched. `claim` serves a marked stream from the row alone (`at < correlated`) instead of probing the event log, which is what moves its cost from O(subscribed streams) to O(rows with work). `NULL` means **unknown**, not "no work": an unmarked row falls back to the has-work probe below, so an install that predates the column behaves exactly as it did and migrates row by row. Only `correlate` may write a mark — the store cannot resolve user-code targets and `notify` is best-effort. Gated by the TCK's `work_set` capability; an adapter that ignores the field stays conformant through the legacy arm.
- **Literal-vs-pattern claim sources**: `claim`'s has-work probe compares a **literal** subscription `source` (no regex metacharacter) to event stream names by **string equality** — index-friendly, and exact so `s1` never matches `s12`. A **pattern** source (carrying regex metacharacters) is compiled and matched against candidate stream names. Adapters that cannot run an arbitrary regex reject a non-portable pattern at `subscribe` time.
- **Retry accounting**: every granted lease increments the stream's retry counter; only `ack` resets it. A timed-out lease reclaimed by any worker therefore counts against the retry budget (see the [concurrency model](./concurrency-model)).
