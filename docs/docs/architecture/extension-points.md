...
The `EventSource.query` callback is typed `(event) => void`, but adapters wrap each invocation in `await Promise.resolve(callback(event))`. TypeScript's "any return ignored when the type says `void`" rule lets the same call site work for both sync (`e => arr.push(e)`, returns `number`) and async (`async e => …`) callbacks. The orchestrator's `iterate` utility (in `libs/act/src/internal/event-sourcing.ts`, alongside `scan`) exploits this seam in two layered mechanisms:

- **Outer pagination loop** ([ACT-1133](https://github.com/Rotorsoft/act-root/issues/817)). `iterate` re-issues `source.query` with `limit: ITERATE_BATCH` (500 by default) and a bumped `after` (forward) / `before` (backward) per batch. Stores that respect `limit` (`PostgresStore`'s `pool.query` honors it natively) hold one batch's worth of rows in memory per round trip — adapter cost stays at O(batch) regardless of total result size. Sources that ignore the filter and stream everything in one call (`CsvFile`) signal the loop to exit by returning more events than the requested limit; they're already memory-safe because they read line-by-line internally. The caller's `filter.limit` (if any) caps the total across batches. Bounded queries with `limit ≤ ITERATE_BATCH` pay exactly one round trip — same cost as the pre-pagination path. No new dependency, no `Query` schema field, no adapter-side branching.
- **Inner mailbox** ([ACT-1128](https://github.com/Rotorsoft/act-root/issues/787)). Within each batch, the producer's callback returns a promise that the bridge resolves only when the consumer pulls the event, turning the sync-callback `query` API into an `AsyncIterable<Committed>` with a 1-slot mailbox. Memory in the bridge proper is bounded to one event, regardless of how aggressively the adapter buffers internally.

Net effect: a million-event PG → CSV transfer holds at most `ITERATE_BATCH` rows in the adapter, one event in the bridge mailbox, and whatever the consumer accumulates downstream — independent of total source size. `iterate` is internal because it's a transport detail; `CsvFile`, `EventSource`, and `EventSink` are the public surface the rest of the framework speaks.
