...
`RestoreOptions` carries two flags as of [ACT-1125](https://github.com/Rotorsoft/act-root/issues/784): `drop_snapshots` (skip `__snapshot__` rows so the next snap policy regenerates them) and `on_progress` (one callback per row — callers throttle/debounce as needed). Two more flags — `drop_closed_streams` and `drop_empty_streams` — are deferred until the source-shape question (one-pass vs. re-iterable factory) is settled. The migration overlay (event-name remap, per-event transform, stream rename) lives in #785.

**Validation is a source operation, not a store operation.** Pre-flighting a backup — checking for duplicate ids, version-contiguity gaps, malformed `created`, negative versions — is something callers do over the source before deciding to invoke `Store.restore`. The framework provides `validateRestoreRow()` (a stateful per-row validator factory exported from `@rotorsoft/act`); callers iterate their source and accumulate `{row, reason}` entries themselves. Adapters never see validation policy — their job is the destructive live write, full stop.
