...
Two consequences worth knowing. A store read **directly** (`Store.query`, outside `IAct`) returns the stored string, since typing is the orchestrator's job — the store is a byte-level surface. And a date nested inside an array or record keeps whatever the adapter returned: those paths are not statically enumerable from the schema, so they are not converted.
