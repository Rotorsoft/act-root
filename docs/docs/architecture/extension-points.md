...
- **Retiring subscriptions separately** (when implemented): `retire(streams)` removes subscription rows and nothing else — it MUST NOT touch the event log. It exists so retirement can be **two steps instead of one transaction**. A store whose event log and subscriptions live on different systems has no transaction spanning both, so it cannot satisfy the atomic-truncate guarantee above; it delegates `truncate` to the log half and implements `retire` against the subscription half. `Act.close` calls it **after** a successful truncate, with the streams seeded with a `__tombstone__` (retired) rather than a `__snapshot__` (restarted, and keeping its subscription). That order is fixed by the orchestrator so no adapter re-derives it: truncate commits the tombstone that stops new events landing, so a crash between the steps orphans a row that claims nothing and is reaped by the next close, whereas the reverse order would leave a live stream with no subscription and silently stop delivery. MUST be idempotent — retiring a stream with no row returns 0, which is what adapters whose `truncate` already removed it will do ([#1527](https://github.com/Rotorsoft/act-root/issues/1527)).
