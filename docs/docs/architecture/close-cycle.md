...
**A failed `Store.defer` write cannot stall the schedule or the cycle.** The write happens during drain finalization, after acks and blocks have landed — so a thrown persist used to be able to drop the rest of the cycle's outcomes, including already-acked close requests that would then never be redelivered. Since #1124 the write is isolated per due-time group: a failure is collected, the cycle finishes (close requests, `acked`/`blocked` lifecycle events, and the other groups' persists all land), and the failure is fed to the circuit breaker — surfacing on the `error` lifecycle event and scheduling the breaker's paced retry probe, which re-drives `settle()` even on the default lane. The affected streams keep **no** `deferred_at` and are deliberately not parked in the local `DeferTimer` either: they stay immediately claimable, so the next drain redelivers, the handler re-throws its `DeferSignal` (the due-time is derivable from the triggering event, so it resolves to the same instant), and the retried persist heals the durable schedule. The failure mode is early redelivery, never a stalled recurrence. The same self-healing covers a worker that crashes between the defer decision and the persist: the pending event has no `deferred_at`, so the first drain after restart (controllers arm at construction) redelivers and re-persists.
