...
Call `Store.query_stats(streams, { exclude: [SNAP_EVENT] })` for the whole target set, plus a second pass excluding `TOMBSTONE_EVENT` as well. The store returns the latest non-snapshot event per stream in a single round trip — on Postgres and SQLite this uses the existing `(stream, version)` index for an index-only path (see [ACT-639](https://github.com/Rotorsoft/act-root/issues/639)). A stream absent from the markers-excluded pass has no domain events left, so a previous close ran to completion and it is skipped (the call doesn't re-tombstone an already-closed stream). A stream that is *present* there but whose non-snap head is a tombstone was guarded and then interrupted — an archive callback threw, or the truncate failed — and is **resumed**: the existing guard's event id is reused, Phase 3 skips re-tombstoning, and Phases 4-6 run normally ([#1389](https://github.com/Rotorsoft/act-root/issues/1389)). That is what makes the retry documented in Phase 5 actually work. This **domain** head drives `maxId` (the safety probe — a subscription advances its watermark on domain events, never snapshots) and `lastEventName` (the restart-seed owner lookup).

A second heads-only pass — `query_stats(streams, {})`, snapshots included — supplies the guard `version`. The tombstone's optimistic lock must expect the stream's *actual* current version, which is one higher when a `__snapshot__` trails the domain head (`snap()` commits it into the next version slot). Sourcing the guard version from the domain head would make Phase 3 expect a stale version and skip the close whenever a terminal commit crossed a `.snap()` boundary ([#1356](https://github.com/Rotorsoft/act-root/issues/1356)). The kept entries become a `Map<stream, { maxId, version, lastEventName }>` consumed by the subsequent phases.
...
That question is only fair about events correlate has already resolved, so **the probe advances the read cursor to the head of the streams being closed before asking**. An uncorrelated tail raises no marks, so every reader would answer "caught up" about it — including the reader that does not exist yet, since the subscription a dynamic resolver would create is itself a product of correlating those events. Refusing to close instead would be worse than useless: the autoclose path fires *from* the event that reaches the terminal state, so its own trigger is routinely uncorrelated, and a retired stream gets no further commits to retry with. The catch-up is bounded (a fixed window, a capped number of passes), and whatever still sits above the cursor afterwards lands in `skipped` — the documented retryable outcome.
...
- **The owning state carries `sensitive()` fields**: `restart: true` is refused for that stream *before* Phase 3, so nothing is written. The seed load is actorless, so every sensitive field would fold to the redaction sentinel and the truncate would then delete the real events and their `pii` sidecars — irreversible. Loading the seed *privileged* is not an alternative: that persists plaintext into `__snapshot__.data`, which `forget_pii` cannot reach (the reason `.snap()` is rejected at build time on such a state). Since the stream cannot be restarted, it is not closed either — the caller asked to keep the aggregate alive, and tombstoning it instead would be strictly more destructive than what was requested. The stream lands in `skipped` and an error is logged. Closing the same stream **without** `restart` works normally.
