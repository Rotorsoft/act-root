...
| A state projection's first-sight fold pairs the loaded `state` with the frontier `load()` returns atomically (`Snapshot.frontier`) — never a separate cache read, which could race a concurrent commit and pair older state with a newer frontier, permanently skipping the intervening events | `cache-and-snapshots.md`; `projections-to-database.md` | `fold-frontier-toctou.spec.ts` → "does not skip events when the cache frontier moves during first-sight load" **(#1204)** |
