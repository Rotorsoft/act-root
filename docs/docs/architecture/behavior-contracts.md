...
| A failed `Store.defer` write never stalls the deferred stream: it is surfaced on the `error` lifecycle event, the stream stays immediately re-claimable (no local park, no `deferred_at`), and redelivery re-throws the `DeferSignal` so the retried persist heals the durable schedule | `close-cycle.md` § Defer, not poll; `recipes/temporal/recurring-timers` § Failure modes | `defer-durability.spec.ts` → "redelivers and heals the schedule when the defer write fails", "keeps the drain armed while a defer write is unhealed" **(#1124)** |
| Close requests and `acked`/`blocked` lifecycle events from a drain cycle survive a failed `Store.defer` write in the same cycle — the persist is isolated per due-time group and never aborts finalization | `close-cycle.md` § Defer, not poll | `defer-durability.spec.ts` → "a close request in the same cycle survives a failed defer write" **(#1124)** |
