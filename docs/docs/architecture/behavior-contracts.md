...
| A snapshot event is only persisted from a contiguous fold; conflicts caused solely by the framework's own `__snapshot__` commits retry transparently without burning the caller's retry budget | `cache-and-snapshots.md` | `optimizations.spec.ts` cadence + `calculator.spec.ts` lifecycle (fail on regression) |
