...
- **`block`** — handler failed past the retry budget (or threw `NonRetryableError`); set `blocked=true`. The stream stays out of `claim()` results until something explicitly unblocks it. Use `app.unblock([stream])` to resume from where the stream stopped (the common case — operator fixed the underlying issue), or `app.reset([stream])` to rebuild from event 0 (projection rebuild, rare).
