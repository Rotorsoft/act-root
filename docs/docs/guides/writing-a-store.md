...
- `subscribe(streams, correlated_at?)` — register streams so they become claimable; each row carries optional `lane` that the adapter UPSERTs on every call (restart-driven re-laning) and an optional `correlated` **work mark** that decides claim eligibility — both covered below. It also carries the **correlate checkpoint**, read from the return and written by the optional second argument
...
`SubscribeInput.correlated` ([#1485](https://github.com/Rotorsoft/act-root/issues/1485))
...
- **`NULL` means unknown, not "no work".** An unmarked row must fall back to
  ...
  marks it. Do not backfill the column — `correlated = MAX(id)` makes every
...
- **The operator surfaces leave the mark alone.** `reset` rewinds `at` to
  `-1` and must not clear `correlated`, or a rebuild would be unclaimable.
  `unblock`, `defer`, and `prioritize` don't touch it either.

The payoff is an index, not a loop. `at < correlated` is a legal partial-index
predicate — immutable, single-row, no cross-row reference — so the correlated
set *is* an index:

```sql
CREATE INDEX <table>_streams_correlated_ix
...
`correlated` and re-enters when correlate raises the mark. One caveat if you
