...
- **Sliced states pass every partial.** `.of(TicketCreation, TicketMessaging, TicketOperations)` — the partials provide typing and event registration only; the fold always runs on the registry-merged full state resolved at `act().build()`, which throws if the projection misses any of the state's events. A projection can never silently fold a subset.
