...
The same constraint reaches one runtime operation: **`app.close({ stream, restart: true })` does not apply to a stream owned by a PII-aware state.** A restart seed *is* a snapshot, so it inherits the problem exactly. The stream lands in `close()`'s `skipped` list with an error logged and nothing mutated — deliberately, because tombstoning a stream the caller asked to keep alive would be strictly more destructive than the operation they requested. Closing the same stream **without** `restart` retires it normally, and `.autocloses({ keep: { days: N } })` is unaffected: a windowed close prunes behind an existing snapshot and writes no seed, so a PII-aware state simply has no qualifying snapshot to prune behind and lands in `skipped` the same way.

None of this is a policy choice about how much PII may be persisted — it is structural. State in a PII-aware reducer chain derives from the actor-gated event view, so there is no actor-independent state to freeze into a snapshot in the first place. Making these states snapshottable means moving gating from the event to the derived state, which is a different framework than the one documented here. That trade was written up and declined in [RFC 1405](https://github.com/Rotorsoft/act-root/blob/master/rfcs/1405-snapshot-pii-sidecar.md); the practical consequence is that a long-lived PII-bearing aggregate replays its full history on a cold read, and if that becomes the bottleneck the lever is a shorter-lived stream design, not a snapshot.
