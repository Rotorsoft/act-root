...
- `InMemoryStore` — the reference store implementation; its TCK spec runs the full store contract with `restore`, `pii_isolation`, `concurrent_claim`, `source_matches`, and `pattern_claim_source` on (`notify` stays off — the in-memory store is single-process by design and does not implement it).
