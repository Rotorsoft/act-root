...
- **`drop_closed_streams: true`** — drop every event from streams that have a `__tombstone__` event (closed via `app.close()`). Scan walks the source once upfront with a tombstone-name filter to collect the closed-stream set, then the main pass drops every event whose stream is in that set (including the tombstones themselves and any pre-close events). Counted in `ScanResult.dropped.closed_streams`. There is no `drop_empty_streams` counterpart — empty streams have zero events and never appear in an event scan.
