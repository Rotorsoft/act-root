...
| --- | --- | --- | --- | --- |
| **A — microbench** | `libs/<pkg>/bench/*.bench.ts` (uses `bench()` from vitest) | `pnpm -F <pkg> bench:micro` (or `pnpm bench:micro` for all) | vitest's stats table (mean / p99 / variance / "X× faster than Y") | Comparing implementations of a tight inner loop (delta vs JSON-patch, snap intervals, drain skip vs no-skip). Quick to run, stable enough for relative comparisons. |
| **B — research script** | `libs/<pkg>/scripts/*.bench.ts` (top-level `await`, ends with `process.exit(0)`) | `npx tsx libs/<pkg>/scripts/<file>.bench.ts` | `console.log` markdown tables | Exploring a question that doesn't fit `bench()` — e.g., multi-worker contention sweeps, schema rewrites under load. Manual, ad-hoc. |
| **C — scenario bench** | `libs/<pkg>/bench/*.bench.ts` (uses `it()` + assertions) | `pnpm -F <pkg> bench:scenarios` (or `pnpm bench:scenarios` for all) | `console.table` summary + pass/fail | Realistic end-to-end scenarios with a regression bound (cross-process notify latency, priority claim ordering, reaction latency). CI runs these on PRs. |

Microbenches and scenario benches share the `bench/` directory because both are vitest-driven; they're separated at the run-command level (`vitest bench` vs `vitest run --config vitest.bench.config.ts`).
...
Both run in parallel across packages with `--aggregate-output` for clean logs.
...
| --- | --- | --- | --- |
| `bench/cache.bench.ts` | A | `load()` performance with snap intervals (10 / 50 / 75 / 100 events / no snap), at stream lengths 50 / 500 / 2000 events | Validates that snapshots help with cold loads. |
| `bench/drain-skip.bench.ts` | A | drain after a non-reactive event (skip optimization) vs reactive event | Documents the [drain-skip optimization](./libs/act/PERFORMANCE.md#drain-skip-for-non-reactive-events-v0240) (~3× faster). |
| `bench/batch-projection.bench.ts` | A | per-event drain vs batched drain at 50 / 200 / 500 events | Documents the [batched projection replay](./libs/act/PERFORMANCE.md#batched-projection-replay) (~10–100× faster). |
| `bench/reaction-latency.bench.ts` | C | commit→reaction latency (p50 / p95 / p99) at idle / 100/sec / 1000/sec on `InMemoryStore` | **ACT-103**. Built-in regression bound: idle p99 < 50 ms. See [Reaction latency](./libs/act/PERFORMANCE.md#reaction-latency-act-103). |
...
| File | Shape | Measures | Notes |
| --- | --- | --- | --- |
| `bench/cache.bench.ts` | A | `load()` perf with snap intervals on PG | PG-specific cache validation. |
| `bench/claim.bench.ts` | A | atomic claim throughput | Validates [`FOR UPDATE SKIP LOCKED`](./libs/act/PERFORMANCE.md#atomic-stream-claiming-v0210). |
| `bench/drain-scale.bench.ts` | A | drain throughput at 100 / 500 streams × 1 / 3 / 5 workers | Horizontal scale shape. |
...
| File | Shape | Measures | Notes |
| --- | --- | --- | --- |
| `bench/delta.bench.ts` | A | act-patch's `delta` vs JSON Merge Patch (RFC 7396) generator | Validates the immutable-deep-merge-patch performance story. |
| `bench/patch.bench.ts` | A | act-patch apply vs RFC 7396 vs json-patch (RFC 6902), sequential 10 patches | Comparison against established standards. |
