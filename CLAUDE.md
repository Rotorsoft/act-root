...
- **`/libs`** — core framework libraries
  ...
  - `@rotorsoft/act-http` — HTTP integrations (umbrella). `webhook` for reaction-driven POST delivery, `receiver` for inbound webhook ingestion, the `sse` subpath for incremental state broadcast (the surface once published standalone as the now-removed `@rotorsoft/act-sse`), plus the auto-generated API subpaths (`trpc`, `hono`, `openapi`) that walk a built `IAct` registry and emit one route per action — guide at [docs/docs/guides/auto-generated-api.md](docs/docs/guides/auto-generated-api.md)
...
- **Blocked-stream recovery — `unblock` resumes, `reset` rebuilds.** A stream blocks on `retry >= maxRetries`, when a handler throws `NonRetryableError` (blocks on first attempt), or when it arrives at claim with `retry` *strictly greater* than `maxRetries` — the budget spent with no error ever raised, which only happens when the lease was lost every round (#1418; the lever there is `leaseMillis`, not `maxRetries`). A claim the *store* ate — a cycle that threw on `fetch`/`block`/`ack` after `claim` already persisted `retry + 1` — is refunded by the drain's per-lane ledger before any block decision reads it, so a transient store fault never blocks a stream and never sends the operator after a lease that was never the constraint (#1592). Recovery path is `app.unblock(input)` — preserves the watermark, stream resumes from where it stopped. `app.reset(input)` is for projection rebuilds: it sets the watermark to `-1` and replays every event. Don't confuse them — using `reset` to "clear a blocked webhook" would re-fire every historical webhook. Both accept either `string[]` or a `StreamFilter` (regex/exact/source/blocked). Use `app.blocked_streams()` to discover what's blocked before recovering. See [error-handling.md § Blocked Streams](docs/docs/concepts/error-handling.md).
...
- **Layering is enforced by a test, not by memory.** `libs/act/test/architecture.spec.ts` fails the build if `internal/` imports `node:async_hooks`, imports a *value* (not a type) from `scoped.ts`, declares module-level mutable state, or re-exports anything from outside `internal/`; if any module other than `scoped.ts` constructs, runs, or reads an `AsyncLocalStorage`; or if `ports.ts` publishes the ports ALS instance (the `Scoped` *type* is public — it types `ActOptions.scoped`; the instance is not). The rule these encode: **`internal/` holds stateless implementations that RECEIVE what they need** (`ReactionDeps.reaction_scope`, `DrainDeps.run_scoped`) and never reach for ambient state; `scoped.ts` owns every ambient context and every operation on one. Each rule exists because it was broken — #1541 put an ALS in `internal/`, and the first "fix" moved the declaration one directory over while `internal/` kept importing and reading it. Relocating a declaration is not decoupling a dependency. If a rule blocks you, the answer is almost always to inject the thing, not to widen the rule.
...
- **Integration helpers live in separate packages, never in core.** HTTP delivery, message-bus forwarders, webhook signers, etc. go in their own `@rotorsoft/act-*` package (precedent: `act-http`, `act-pino`, `act-pg`, `act-sqlite`, `act-tck`, `act-patch`). Core stays governed by `STABILITY.md`.
