...
| `drainStatus` | query | Drain pipeline health: aggregates, blocked streams, leases, watermark histogram, priority + lane counts. A subscription counts as *retired* only when its target names a retired event stream **and** it has nothing pending (`at >= correlated_at`) — the two names live in different namespaces, so a live blocked subscription is never filed away behind an unrelated retired stream (#1553) |
| `browse_correlators` | query | Who is looking for new work and how far each has read, from the store's own `*_correlated` table. Empty for an in-memory store and for stores predating #1532; a read that *fails* raises instead (#1554) |
...
- **A failed read is never data**: every read procedure raises on failure and the views render "couldn't read" — no zeroed dashboard, no empty list, no default lane or priority standing in for a value nobody read (#1552 / #1554 / #1566). The one deliberate exception is a *missing* correlators table, which is a real answer ("this store has nothing to record"), not a failure. Discovery probes are the other exception by design: a candidate that fails a probe is not a store, which is what the scan was asking.
